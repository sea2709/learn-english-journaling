...
6. In-text `SuggestionHighlight` marks open the note (double-click on desktop, tap on touch; bottom sheet on small/coarse pointers). After **Edit** in a note, taps on that same highlight (including other lines of a wrapped span) place the caret while the editor is focused; taps on other highlights open their notes instead. When the editor is not focused, highlight taps always open notes. On the mobile sheet, tapping another highlight under the backdrop switches notes in one tap (hit-tests marks beneath the overlay). Taps on non-highlighted text still move the caret normally. `SuggestionRow` under the paragraph lists the same notes; stale edits are flagged via `isParagraphStale()`.
...
1. User expands a paragraph suggestion and asks a follow-up in `SuggestionRow` (paragraph notes only; Review drawer stays view-only).
...
1. User opens the **Review** drawer from the topbar.
...
- Change password (signed in): email-identity users open **Change password** from the Account menu → `ChangePasswordForm` verifies via `signInWithPassword` with the current password, then `updateUser({ password })`.
