...
6. In-text `SuggestionHighlight` marks open the note (double-click on desktop, tap on touch; bottom sheet on small/coarse pointers). After **Edit** in a note, taps on that same highlight (including other lines of a wrapped span) place the caret while the editor is focused; taps on other highlights open their notes instead. When the editor is not focused, highlight taps always open notes. Taps on non-highlighted text still move the caret normally. `SuggestionRow` under the paragraph lists the same notes; stale edits are flagged via `isParagraphStale()`.
